...
fn handle_embedded_file(path: &str, i18n: I18n) -> HttpResponse {
    match Asset::get(path) {
        Some(content) => HttpResponse::Ok()
            .content_type(from_path(path).first_or_octet_stream().as_ref())
            .body(content.data.into_owned()),
        None => HttpResponse::NotFound().body(format!("404 {}", i18n.text("not_found"))),
    }
}

#[actix_web::get("/static/{_:.*}")]
pub async fn static_resources(path: web::Path<String>, request: HttpRequest) -> impl Responder {
    handle_embedded_file(path.as_str(), I18n::from_request(&request))
}
